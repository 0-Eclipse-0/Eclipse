print("Enter the Filename (No Extension)")
print(">>> ")
userInp = gets.chomp();
name = userInp
extension = ".ec"
...
File.open(filename, "r") do |f|
 ...
 ### End Ext StyleSheet
   ...
   if errMsgVal == 1
    noErrors = false;
    target.puts("There are errors on line(s): ")
    target.print("[", lineNumber, "] ")
    target.puts(line)
    errMsgVal = errMsgVal + 1
   else
    noErrors = false;
    target.print("[", lineNumber, "] ")
    target.puts(line)
 ...
 if (noErrors == false)
   puts "**There are errors you may need to fix.**"
 else
   puts "There are no errors. That doesn't mean your code will work. It just means that all tags you used are valid."
   target.write("There are no errors. That doesn't mean your code will work. It just means that all tags you used are valid.")
 end
end
